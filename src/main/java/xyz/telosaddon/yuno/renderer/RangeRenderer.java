...
public class RangeRenderer {
	...
	public static void init() {
		WorldRenderEvents.END_MAIN.addPhaseOrdering(Event.DEFAULT_PHASE, RENDER_IDENTIFIER);
		WorldRenderEvents.END_MAIN.register(RENDER_IDENTIFIER, RangeRenderer::draw);
	}
	...
	private static void draw(WorldRenderContext context) {
		...
		assert player != null;
		var matrixStack = context.matrices();
		assert matrixStack != null;


		float tickDelta = context.gameRenderer().getCamera().getLastTickProgress();
		...
		mainRangeFeature.draw(tickDelta, context.matrices(), vertexConsumers, player, isMainRangeGreater ? 0 : frontDy);
		offHandRangeFeature.draw(tickDelta, context.matrices(), vertexConsumers, player, isMainRangeGreater ? frontDy : 0);
